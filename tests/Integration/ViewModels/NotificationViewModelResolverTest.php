<?php

declare(strict_types=1);

use App\Actions\AcceptVaultCollaboration;
use App\Actions\CreateVaultCollaboration;
use App\Actions\DeclineVaultCollaboration;
use App\Models\User;
use App\Models\Vault;
use App\ViewModels\NotificationViewModelResolver;

it('resolves a VaultCollaborationInvitationReceived notification model into a view model array', function (): void {
    [$user1, $user2] = User::factory(2)->create();
    $vault = Vault::factory()->for($user1)->create();

    app(CreateVaultCollaboration::class)->handle($vault, $user2);

    $notification = $user2->notifications()->first();
    $array = NotificationViewModelResolver::resolve($notification);

    expect($array)->toBe([
        'id' => $notification->id,
        'type' => class_basename($notification->type),
        'data' => [
            'user_name' => $user1->name,
            'vault_id' => $vault->id,
            'vault_name' => $vault->name,
        ],
    ]);
});

it('resolves a VaultCollaborationAccepted notification model into a view model array', function (): void {
    [$user1, $user2] = User::factory(2)->create();
    $vault = Vault::factory()->for($user1)->create();

    app(CreateVaultCollaboration::class)->handle($vault, $user2);
    app(AcceptVaultCollaboration::class)->handle($vault, $user2);

    $notification = $user1->notifications()->first();
    $array = NotificationViewModelResolver::resolve($notification);

    expect($array)->toBe([
        'id' => $notification->id,
        'type' => class_basename($notification->type),
        'data' => [
            'user_name' => $user2->name,
        ],
    ]);
});

it('resolves a VaultCollaborationDeclined notification model into a view model array', function (): void {
    [$user1, $user2] = User::factory(2)->create();
    $vault = Vault::factory()->for($user1)->create();

    app(CreateVaultCollaboration::class)->handle($vault, $user2);
    app(DeclineVaultCollaboration::class)->handle($vault, $user2);

    $notification = $user1->notifications()->first();
    $array = NotificationViewModelResolver::resolve($notification);

    expect($array)->toBe([
        'id' => $notification->id,
        'type' => class_basename($notification->type),
        'data' => [
            'user_name' => $user2->name,
        ],
    ]);
});
